...
# alone rather than overridden with an empty string -- `git describe`
# fails whenever the build context has no usable git repository, which
# includes building from a git worktree, where .git is a file pointing
# outside the context. That produced binaries reporting a bare "v".
VERSION ?=
